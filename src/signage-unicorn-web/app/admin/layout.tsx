...
export default function AdminLayout({
    ...
    // Workflow Order: Dashboard -> Media -> Playlists -> Devices -> Stats -> Logs -> Settings -> Guide
    const menuItems = [
    ...
    React.useEffect(() => {
        ...
        const fetchStats = async () => {
            try {
                ...
                if (res.success && res.data) {
                    setStats(res.data);
                    // Check for new logs to show red dot
        ...
        if (allowStatsLoad.current) {
            ...
            fetchStats();
            interval = setInterval(fetchStats, 5000); // Faster update for Latency
        }
    ...
    const roleLabel = (role: string) => {
        if (role?.toLowerCase() === 'admin') return 'Network Authority';
        if (role?.toLowerCase() === 'editor') return 'Content Manager';
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-[var(--background)] text-[var(--foreground)] font-sans transition-colors duration-300">
            ...
            <aside className={`${sidebarCollapsed ? 'w-24' : 'w-72'} glass-panel border-r border-white-[0.05] flex flex-col z-30 relative overflow-hidden transition-all duration-500 ease-in-out`}>
                ...
                <div className={`${sidebarCollapsed ? 'p-2' : 'p-6'} mt-auto relative z-10`}>
                    <Link href="/admin/settings" className={`glass-card ${sidebarCollapsed ? 'p-3' : 'p-4'} rounded-2xl border border-border/50 hover:border-border flex items-center ${sidebarCollapsed ? 'justify-center' : 'gap-4'} group cursor-pointer transition-all`}>
                        <div className="w-10 h-10 rounded-full bg-accent-purple/10 border border-border flex items-center justify-center text-lg group-hover:border-accent-cyan/50 transition-colors flex-shrink-0">
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                ...
                {/* Visual Header */}
                <header className="h-20 border-b border-border/30 flex items-center justify-between px-10 bg-background/80 backdrop-blur-xl z-20 transition-colors duration-300">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-2 px-3 py-1 rounded-lg bg-green-500/5 border border-green-500/20 text-green-400 text-xs font-black tracking-widest uppercase">
                        ...
                        <div className="text-xs text-muted-foreground hidden md:flex gap-4 uppercase font-bold tracking-widest">
                    ...
                    <div className="flex items-center gap-8">
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                ...
                                className="w-10 h-10 rounded-xl bg-muted/20 border border-border/50 flex items-center justify-center hover:bg-muted/40 transition-colors relative group"
                                title={`Current Size: ${fontSize.toUpperCase()}`}
                            >
                                <div className="flex items-end gap-0.5">
                                    <span className={`font-bold transition-all ${fontSize === 'small' ? 'text-accent-cyan text-sm' : 'text-muted-foreground text-xs'}`}>A</span>
                                    <span className={`font-bold transition-all ${fontSize === 'medium' ? 'text-accent-cyan text-base' : 'text-muted-foreground text-xs'}`}>A</span>
                                    <span className={`font-bold transition-all ${fontSize === 'large' ? 'text-accent-cyan text-xl' : 'text-muted-foreground text-xs'}`}>A</span>
                                </div>
                            </button>
                            ...
                            <button
                                onClick={toggleTheme}
                                className="w-10 h-10 rounded-xl bg-muted/20 border border-border/50 flex items-center justify-center text-lg hover:bg-muted/40 transition-colors"
                            >
                                {theme === 'dark' ? '🌙' : '☀️'}
                            </button>
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className={`w-10 h-10 rounded-xl bg-muted/20 border border-border/50 flex items-center justify-center text-xl transition-all ${stats?.recentAlerts?.length ? 'hover:bg-accent-cyan/10 hover:border-accent-cyan animate-pulse-slow' : 'opacity-50 cursor-not-allowed'}`}
                                >
                                    <Bell size={20} />
                                ...
                                {isNotificationsOpen && stats?.recentAlerts && (
                                    <div className="absolute top-14 right-0 w-80 bg-card-solid rounded-2xl border border-border shadow-2xl z-50 animate-in fade-in slide-in-from-top-2 duration-300 overflow-hidden">
                                            ...
                                            {stats.recentAlerts.map((log, idx) => (
                                                <div key={idx} className="p-3 hover:bg-muted/30 rounded-xl transition-colors group">
                                                    <div className="flex justify-between items-center mb-1">
                                                        <span className={`text-xs font-black uppercase px-1.5 rounded ${log.logType === 'ERROR' ? 'bg-red-500/10 text-red-400' : 'bg-accent-cyan/10 text-accent-cyan'}`}>{log.logType}</span>
                                                        <span className="text-xs text-muted-foreground font-mono">{new Date(log.createdAt).toLocaleTimeString()}</span>
                                                    </div>
                                                    <p className="text-xs text-foreground/80 leading-snug">{log.message}</p>
                                                    <div className="text-xs text-muted-foreground mt-1 uppercase font-bold tracking-tighter">Source: {log.source}</div>
                                                </div>
                                            ))}
                ...
                {/* --- Scrollable Content --- */}
                <main className="flex-1 overflow-y-auto custom-scrollbar relative page-transition">
